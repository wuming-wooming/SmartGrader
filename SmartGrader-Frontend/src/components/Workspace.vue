    ...
    <el-main class="main-content">
      <el-card class="workspace-card">
        <el-steps :active="currentStepIndex" finish-status="success" align-center class="steps">
          <el-step title="上传作业" />
          <el-step title="图片清洗" />
          <el-step title="ocr 与 切题" />
          <el-step title="模型批改" />
          <el-step title="查看报告" />
        </el-steps>

        <div class="view-container">
          <UploadView v-if="workspaceStep === 'upload'" @uploaded="handleUploaded" />
          <ImageCleanView
            v-else-if="workspaceStep === 'ocr_processing'"
            :taskId="ocrTaskId"
            :rawImageUrl="rawImageUrl"
            @complete="handleOcrComplete"
          />
          <ProcessingView 
            v-else-if="workspaceStep === 'grading_processing'" 
            :step="workspaceStep" 
            :taskId="gradingTaskId"
            @grading-complete="handleGradingComplete"
          />
          <OcrConfirmView 
            v-else-if="workspaceStep === 'ocr_confirm'" 
            :taskId="ocrTaskId" 
            :rawImageUrl="rawImageUrl"
...
import ImageCleanView from './ImageClean.vue'
...
const workspaceStep = ref<'upload' | 'ocr_processing' | 'ocr_confirm' | 'grading_processing' | 'result'>('upload')
...
const currentStepIndex = computed(() => {
  switch (workspaceStep.value) {
    ...
    case 'ocr_confirm': return 2
    case 'grading_processing': return 3
...
const handleUploaded = (taskId: number, rawUrl: string) => {
  ocrTaskId.value = taskId
  rawImageUrl.value = rawUrl
  workspaceStep.value = 'ocr_processing'
}

const handleOcrComplete = () => {
  workspaceStep.value = 'ocr_confirm'
}

const handleGradingSubmit = (assignmentTaskId: number) => {
  gradingTaskId.value = assignmentTaskId
  workspaceStep.value = 'grading_processing'
}
