        ...
        <template v-if="result">
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="16">
                        <el-card shadow="hover" class="comment-card">
                            ...
                            <div class="comment-content">{{ result.summary_comment }}</div>
                        </el-card>
                    </el-col>
                </el-row>
            </div>
            ...
            <div class="details-section">
                <el-collapse v-model="activeNames">
                    <el-collapse-item v-for="(item, index) in result.details" :key="index" :name="index">
                        <template #title>
                        ...
                        <div class="item-content">
                            <div class="image-box" v-if="item.question_image">
                                <el-image :src="getImageUrl(item.question_image)"
...
const fetchResult = async () => {
    ...
    try {
        ...
        const details = (data.questions || []).map((q: any) => {
            const isCorrect = q.is_correct === 1 || q.is_correct === true;
            const finalScore = (isCorrect && (!q.score || q.score === 0)) ? (q.full_score || 10) : (q.score || 0);
            return {
        ...
        const computedTotalScore = details.reduce((sum: number, item: any) => sum + item.score, 0);
        const reportTotalScore = data.report?.total_score || 0;

        result.value = {
            total_score: reportTotalScore > 0 ? reportTotalScore : computedTotalScore,
