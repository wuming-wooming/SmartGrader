...
class DenoiseStep(PipelineStep):
    ...
    def process(self, image: np.ndarray) -> np.ndarray:
        method = self.kwargs.get("method", "gaussian")
        if method == "gaussian":
...
class CropStep(PipelineStep):
    ...
    def process(self, image: np.ndarray) -> np.ndarray:
        ...
        h, w = gray.shape[:2]
        img_area = w * h
        min_area_ratio = float(self.kwargs.get("min_area_ratio", 0.0005))
        max_area_ratio = float(self.kwargs.get("max_area_ratio", 0.90))
        ...
        pad = int(self.kwargs.get("padding", 40))
        pad_y = int(self.kwargs.get("padding_y", 60))

        valid = []
        for c in contours:
            x, y, cw, ch = cv2.boundingRect(c)
            area = cw * ch
            if area > img_area * min_area_ratio and area < img_area * max_area_ratio:
                valid.append(c)
        ...
        if content_w * content_h > img_area * min_content_ratio and content_w * content_h < img_area * max_content_ratio:
            x1 = max(0, x_min - pad)
...
class EnhanceStep(PipelineStep):
    ...
    def process(self, image: np.ndarray) -> np.ndarray:
        method = self.kwargs.get("method", "adaptive_threshold")
...
class ImagePipeline:
    ...
    def from_config(cls, steps_str: str | None = None) -> "ImagePipeline":
        """从配置字符串构建管道，如 "grayscale,threshold,crop,enhance" """
        ...
        for name in step_names:
            ...
            if cls_type:
                steps.append(cls_type(name=name))
        return cls(steps)
